...
This file provides guidance for using Claude to commit changes following the MacMock project's Commit Workflow.
...
**Example:**
`amitraikwar/MAC-18/add-window-resize`
...
When using Claude to commit changes:
...
printf "3\nMAC-5\nadd spotlight search\n- Implement fuzzy search using Fuse.js across desktop apps.\n- Wire up keyboard shortcut (Cmd+Space) to open Spotlight modal.\n- Adhere to requirements specified in MAC-5 for search feature.\n\n" | make commit
```
...
# Enter ticket number as scope (e.g. MAC-5)
...
- Reference the Jira ticket number in the scope (e.g., `feat(MAC-5): ...`)
