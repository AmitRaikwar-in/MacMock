...
These rules govern all TypeScript and React component code within the **MacMock (AR Mac Portfolio)** repository to ensure strict type safety, clean folder structures, uniform naming conventions, and compliance with the project's UI standards.
...
- **Chakra UI v2 Only**: We **cannot use UI components other than what is offered by Chakra UI v2** for visual controls, inputs, layout grids, or structure. Do not import UI controls from other component libraries.
...
- **Local SVG Components**: Icons are defined as individual `.tsx` files under `src/assets/icons/`. Always use these local SVG React components for icons across the codebase.
- **No Third-Party Icon Libraries**: Do not import from `react-icons`, `lucide-react`, or any other icon package unless explicitly approved. The project's icon system lives entirely in `src/assets/icons/`.
- **Adding New Icons**: When a new icon is needed, create a new `.tsx` file in `src/assets/icons/` exporting a typed React arrow-function component. Export it via the directory's `index.ts`.

### 11. Theming Compatibility

All components **must** follow the project's dark-mode-first design system powered by **Chakra UI v2**.

- **Dark Mode First**: MacMock is a dark-themed application. All components must look correct with the dark color mode active.
- **Chakra UI v2 Theming API**: Use Chakra UI v2's `extendTheme`, `colorMode`, and `useColorModeValue` APIs. Note that Chakra UI v2 does NOT have a semantic token system like v3 — use `useColorModeValue` to branch between light/dark color values when needed.
- **No Raw Hex in Props**: Avoid hardcoded hex strings directly in Chakra component props where a theme color or `useColorModeValue` value can be used instead.
- **Glassmorphism Patterns**: Translucent panels use `backdropFilter: "blur(...)"` and semi-transparent `bg` values (e.g., `rgba(255,255,255,0.1)`). This is acceptable for macOS-style UI effects.
- **Framer Motion for Animations**: Use `framer-motion` for all window, screen, and UI transition animations. Do not use CSS `animation` or `transition` properties for component-level motion.
...
#### ❌ Incorrect — hardcoded hex in a Chakra prop without color mode awareness:
...
export const WindowPanel = () => {
  return (
    // ❌ Hardcoded hex ignores color mode
...
#### ✅ Correct — use `useColorModeValue` for Chakra UI v2 color mode support:

```tsx
import { Box, Text, useColorModeValue } from '@chakra-ui/react';

export const WindowPanel = () => {
  const bg = useColorModeValue('rgba(255,255,255,0.8)', 'rgba(30,30,30,0.85)');
  const color = useColorModeValue('gray.900', 'whiteAlpha.900');

  return (
    // ✅ Responds correctly to light/dark color mode
    <Box bg={bg} color={color} backdropFilter="blur(12px)">
      <Text>Finder</Text>
    </Box>
  );
};
...
#### ✅ Correct — glassmorphism panel (macOS style):

```tsx
import { Box } from '@chakra-ui/react';

export const GlassPanel = () => (
  // ✅ Translucent glassmorphic background with blur
  <Box
